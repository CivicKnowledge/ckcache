...
class FsCache(Cache):
    ...
    def path(self, rel_path, propagate=True, missing_ok=False, **kwargs):
        ...
        if self.upstream and propagate:
            return self.upstream.path(
                rel_path, missing_ok=missing_ok, **kwargs)
    ...
    def cache_dir(self):
        ...
        if not os.path.isdir(cache_dir):
            raise ConfigurationError(
                "Cache dir '{}' is not valid".format(cache_dir))
...
class FsLimitedCache(FsCache):
    ...
    def __repr__(self):
        return "FsLimitedCache: dir={} size={} upstream=({})".format(
            self.cache_dir, self.maxsize, self.upstream)
...
class FsCompressionCache(Cache):
    ...
    def get_stream(self, rel_path, cb=None):
        ...
        # The fallback will try to access the stream without compresion if the
        # compression read fails
    ...
    def get(self, rel_path, cb=None):
        ...
        if not source:
            raise Exception(
                'Failed to get a source for rel_path {} for {}, upstream {} '.format(
                    rel_path,
                    self,
                    self.upstream))
