    ...
    def put_stream(self, rel_path,  metadata=None, cb=None):
        ...
        class flo(IOBase):
            ...
            def close(self):
                ...
                submit_task(rel_path, str(self._upstream), StringIO(self._buffer.getvalue()))
...
class LoggingCache(PassthroughCache):
    ...
    def write_record(self, rel_path):
        ...
        path = os.path.join("_log", str(int(t*1000))+'_'+hashlib.md5(rel_path).hexdigest() )

        with self.upstream.put_stream(path) as s:
            s.write(json.dumps(dict(path=rel_path, cache=str(self.upstream), time=t)))
    ...
    def __repr__(self):
        return str(self.upstream) + (';' if '#' in str(self.upstream) else '#') + 'record'
